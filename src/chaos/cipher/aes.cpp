...
static void setRoundKey(uint8_t *key,uint8_t roundkey[11][16]){
  for(int i=0;i<16;i++) roundkey[0][i] = key[i];
  uint8_t tmp[4];
  for(int i=1;i<=10;i++){
    for(int j=0;j<3;j++) tmp[j] = roundkey[i-1][13+j];
    tmp[3] = roundkey[i-1][12];
    for(int j=0;j<4;j++) tmp[j] = sbox[tmp[j]];
    tmp[0] ^= rc[i];
    for(int j=0;j<4;j++) roundkey[i][j] = roundkey[i-1][j]^tmp[j];
    for(int j=4;j<16;j++) roundkey[i][j] = roundkey[i-1][j]^roundkey[i][j-4];
  }
}

static uint8_t multi(uint8_t a, uint8_t b){
  if (a == 0 || b == 0) return 0;
...
void encrypt(uint8_t *key, uint8_t *inb, uint8_t *outb) {
  ...
  for(int r=1;r<=10;r++){
    ...
    for(int i=0;i<16;i++) outb[i] ^= roundkey[r][i];
  }
}
